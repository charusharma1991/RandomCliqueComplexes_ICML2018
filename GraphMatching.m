function err =  GraphMatching(data)
p=0.7;
k=8;
N = length(data);
P={};
for m=1:1
    for n=m+1:N
        ...
        nv1 = length(data1);
        nv2 = length(data2);
        [G1,G2] = ERGraph_Occ(data1,data2,nv1,nv2,p,k);
        [~,cl1,~] = k_clique(4,G1);
        [~,cl2,~] = k_clique(4,G2);
        ...
        nodes{1}=(1:nv1)';
        for i=2:3
            if length(cliques1)==length(cliques2) || (i<=length(cliques1) && i<=length(cliques2))
        ...
        [P{m}{n},A{m}{n},ptR{m},ptC{n},tim]=matchHasse(H1,H2,cliques1,cliques2,coord1,coord2,nodes,nv1,nv2);
