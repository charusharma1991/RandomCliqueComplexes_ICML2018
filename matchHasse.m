function [P,A,ptR,ptC,tim] = matchHasse(H1,H2,cliques1,cliques2,coord1,coord2,nodes,nv1,nv2)
node=[];
...
for i=1:length(node)
    s1=sum(node(1:i-1));
    for j=1:node(i)
        if length(nodes)==length(cliques1) && ismember(nodes{i}(j,:),cliques1{i},'rows')
            [~,id] = ismember(nodes{i}(j,:),cliques1{i},'rows');
            c1(s1+j,:)=coord1{i}(id,:);
        end
        if length(nodes)==length(cliques2) && ismember(nodes{i}(j,:),cliques2{i},'rows')
...
for i=1:2
    ...
    [H{i}, MNeighborhood{i}, MNeighborCoeff{i}] = Get_Reconstruction_Coeff( d, aa );
    if ~isempty(ptR{i})
